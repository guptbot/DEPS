...
class Multitask(PolicyManagerBase):
    def __init__(self, *args, **kwargs):
        ...
        if not self.args.use_language:
            raise Exception("should provide language to image in multitask")
    ...
    def get_actions(self, batch, network_bundle, sample_traj, robot_states, old_concatenated_traj):
        ...
        if not self.args.image_to_subpolicy:
            raise Exception("when using multitask baseline, should give subpolicy access to the input impage")
        else:
        ...
        selected_a = network_bundle.primitive_policy_set[0].forward(subpolicy_input, continuous_param, skill_tensor)
    ...
    def update_policies(self, batch, network_bundle, act_dict):
        ...
        self.upload_log({"policy loss": torch.mean(subpolicy_loss).cpu().detach().numpy(),
                         ...
                         "gmm avg variance": avg_variance.detach()}, step=self.counter)
    ...
    def get_rollout_action(self, network_bundle, sample_traj, robot_state, lang_embedding):
        ...
        selected_a = network_bundle.primitive_policy_set[0].get_actions(subpolicy_input, z, k)
