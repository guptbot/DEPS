...
class Deps(PolicyManagerBase):
    ...
    def get_actions(self, batch, network_bundle, sample_traj, robot_states, old_concatenated_traj):
        ...
        # No gradient from the variational network to the image encoder, since the variational network is not used at inference time.
        variational_k, variational_z = network_bundle.variational_policy.forward(
            input=old_concatenated_traj, task_indices=task_idx, task_lengths=batch["task_length"])
        ...
        for skill_idx in range(self.args.number_policies):
            ...
            actions, uncorrected_actions = network_bundle.primitive_policy_set[0].forward(sample_traj.detach(), robot_states, continuous_param, skill_tensor)
        ...
        latent_k = network_bundle.discrete_policy.forward(input=sample_traj, task_indices=task_idx)
        latent_z = network_bundle.continuous_policy.forward(input=sample_traj, task_indices= task_idx)
    ...
    def calculate_cloning_loss(self, batch, network_bundle, sample_traj, robot_states, old_concatenated_traj):
        ...
        # Weight the loss for each discrete skill by the probability of sampling that discrete skill from the variational network
    ...
    def update_policies(self, batch, network_bundle, act_dict):
        ...
        if self.args.mask_latent_to_var or self.args.mask_z_to_var:
    ...
    def visualize_learned_skills(self, network_bundle):
        ...
        for (i, batch) in enumerate(self.task_dataloader):
            with torch.no_grad():
                ...
                task_emb = self.get_one_hot_task_tensor(batch)
                variational_k, variational_z = network_bundle.variational_policy.forward(
                    input=old_concatenated_traj, task_indices=task_emb, task_lengths=batch["task_length"])
    ...
    def get_rollout_action(self, network_bundle, sample_traj, robot_state, task_index):
        ...
        latent_k = network_bundle.discrete_policy.forward(input=sample_traj, task_indices=task_index)[0]
        latent_z = network_bundle.continuous_policy.forward(input=sample_traj, task_indices=task_index)[0]
