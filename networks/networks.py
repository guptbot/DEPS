...
from networks.resnet import DataAugGroup, TranslationAug, BatchWiseImgColorJitterAug
...
class VariationalNetwork(torch.nn.Module):
    def __init__(self, args):
        ...
        self.language_input_layer = torch.nn.Linear(768, 32)
        self.lstm = torch.nn.GRU(input_size=args.input_size + 32, hidden_size=args.hidden_size,
    ...
    def forward(self, input, language_input, task_lengths):
        # Concatenate language embedding with state+action trajectory
        language_input = language_input.unsqueeze(1).repeat(1, input.shape[1], 1)
        language = self.hidden_activation(self.language_input_layer(language_input))
        input = torch.cat((input, language), dim=-1)
...
class DiscretePolicyNetwork(torch.nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        # Layers
        self.compression_layer = torch.nn.Linear(args.language_dim, args.hidden_size)
        ...
        self.subpolicy_output_layer1 = torch.nn.Linear(2 * args.hidden_size, args.hidden_size)
        self.subpolicy_output_layer2 = torch.nn.Linear(args.hidden_size, args.number_policies)
        # Activation Funcs
        self.hidden_activation = F.relu

    def forward(self, input, language):
        b, s, _ = input.size()
        language = self.compression_layer(language)
        language = language.reshape(b, 1, -1).repeat(1, s, 1)
        #concat_input_lang = torch.cat((input, language), dim=-1)
        outputs, _ = self.lstm(input)
        concat_output = torch.cat((outputs, language), dim=-1)
...
class ContinuousPolicyNetwork(torch.nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        # Layers
        self.compression_layer = torch.nn.Linear(768, args.hidden_size)
        self.lstm = torch.nn.GRU(input_size=args.state_dim, hidden_size=args.hidden_size,
                                 num_layers=args.variational_latent_num_layers, batch_first=True, bidirectional = False).to(device)
        for name, param in self.lstm.named_parameters():
            if 'bias' in name:
...
# Low-level policy: outputs GMM action distribution conditioned on state and skill parameters
class StochasticLowLevelPolicyNetwork(torch.nn.Module):
    def __init__(self, args, input_size):
        ...
        self.markov_layer = torch.nn.Linear(input_size, args.hidden_size_primitive)
...
class StochasticLowLevelPolicyNetworkWithCompression(torch.nn.Module):
    ...
    def forward(self, input, robot_state, z_vals, skill_idx):
        ...
        dist = self.GMM_head(compressed_action_embedding)
        corrected_dist = self.GMM_head(compressed_action_embedding.detach() + corrected_action_embedding)
    ...
    def get_actions(self, input, robot_state, z_vals, skill_idx):
        with torch.no_grad():
            ...
            corrected_action, action = corrected_dist.sample().detach(), dist.sample().detach()
...
# Data augmentation and pretrained encoders
color_aug = BatchWiseImgColorJitterAug(input_shape = None, brightness = 0.3, contrast =  0.3, saturation = 0.3, hue = 0.3, epsilon = 0.1)
translation_aug = TranslationAug(input_shape = [3, 128, 128], translation = 8)
IMG_AUG = DataAugGroup((color_aug, translation_aug))
IMAGE_ENCODERS = {
    'agentview_rgb': nn.DataParallel(ResnetEncoder(pretrained=False, freeze=False, remove_layer_num = 4, no_stride = False,
                                     language_fusion = 'film', input_shape = [3, 128, 128],
                                     output_size = 64, language_dim = 768)).to(device),
    'eye_in_hand_rgb' : nn.DataParallel(ResnetEncoder(pretrained=False, freeze=False, remove_layer_num = 4, no_stride = False,
                                        language_fusion = 'film', input_shape = [3, 128, 128],
                                        output_size = 64, language_dim = 768)).to(device)}

LANGUAGE_ENCODER = MLPEncoder(input_size = 768, hidden_size = 128, output_size = 32, num_layers = 1).to(device)
